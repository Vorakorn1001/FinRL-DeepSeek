...
TRAINED_MODEL_DIR = "ppo_mini_models"
...
dataset = load_dataset("csv", data_files="datasets/train_data_2013_2018.csv")
...
parser.add_argument('--stock', type=str, default='AAPL', help='Stock symbol (default: AAPL)')
parser.add_argument('--hid', type=int, default=64)  # Updated to match the first hidden size in ac_kwargs
parser.add_argument('--l', type=int, default=3)  # Updated to match the number of layers in ac_kwargs
...
parser.add_argument('--steps', type=int, default=20000)  # Updated to match steps_per_epoch in ppo
...
def ppo(
    ...
    seed=42,
    steps_per_epoch=20000,  # Larger batch size for better gradient estimation
        ...
        for t in range(local_steps_per_epoch):
            ...
            if terminal or epoch_ended:
                ...
                if timeout or epoch_ended:
                    ...
                    v = 0
                buf.finish_path(v)
                # always store EpRet and EpLen even if trajectory didn't finish
...
trained_ppo=ppo(lambda : env_train, actor_critic=MLPActorCritic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), gamma=args.gamma,
        seed=args.seed, steps_per_epoch=args.steps, epochs=args.epochs,
        logger_kwargs=logger_kwargs)

model_path = TRAINED_MODEL_DIR + f"/agent_mini_ppo_100_epochs_20k_steps_64x3_{stock}.pth"
torch.save(trained_ppo.state_dict(), model_path)
# mock save file
# os.makedirs(os.path.dirname(model_path), exist_ok=True)
# torch.save("mock_model_state_dict", model_path)
