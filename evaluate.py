...
MINI_PPO_MODEL_DIR = 'ppo_mini_models'
MINI_CPPO_MODEL_DIR = 'cppo_deepseek_mini_models_70'
...
def plot_cumulative_returns(result, metrics, strategies, benchmark,
                            save_path="cumulative_returns.png", dpi=300):
    """
...
def load_specialized_models(stock_list, model_dir=MINI_PPO_MODEL_DIR):
    """
...
def load_mini_specialized(observation_space, action_space, stock_ticker=None, specialized_models=None):
    ...
    try:
        ...
        mini_model_path = os.path.join(MINI_PPO_MODEL_DIR, 'agent_ppo_mini.pth')
...
def load_cppo_specialized_models(stock_list, model_dir=MINI_CPPO_MODEL_DIR):
...
def adaptive_specialized_flow():
    ...
    cppo_models = load_specialized_models(stock_list, model_dir=MINI_CPPO_MODEL_DIR)
...
def full_specialized_flow():
    """
    Full specialized flow that uses specialized models for all stocks throughout the entire period,
...
def flow():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        # Add our new specialized models approach
    ...
    result = pd.DataFrame(
        {
            ...
            "Mini PPO": df_full_specialized_portfolio,  # Add the new non-segmented strategy
