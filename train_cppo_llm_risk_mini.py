...
import os
import time
TRAINED_MODEL_DIR = "cppo_deepseek_mini_models"
...
parser.add_argument('--model_path', type=str, default=None, 
                    help='Path to pretrained model for continuing training')

args = parser.parse_args()

# from Huggging Face :
dataset = load_dataset("csv", data_files="datasets/train_data_deepseek_risk_2013_2018.csv")
...
def cppo(env_fn,
         ...
         model_path=None,):  # CVaR clipping ratio
...
#            llm_risks_weights = llm_risks_weights.flatten() 
     ...
     #       print("llm risks weights are  : " + str(llm_risks_weights))
      ...
      #      print("prices length is: " + str(prices.shape))
        ...
        logger.log_tabular('Now', time.strftime("%H:%M:%S", time.localtime()))
...
trained_cppo=cppo(lambda : env_train, actor_critic=MLPActorCritic,model_path=args.model_path,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), 
        seed=args.seed, logger_kwargs=logger_kwargs)


# Save the model
model_path = TRAINED_MODEL_DIR + f"/agent_cppo_deepseek_70_epochs_20k_steps_64x2{stock}.pth"
